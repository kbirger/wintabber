...
| `WinTabber.Interop/UacHelper.cs:17,21` | `OpenProcessToken`, `GetTokenInformation` (`advapi32.dll`) | **Yes** — both are standard, documented Win32 APIs | **Migrate to CsWin32** — add both to `WinTabber.Interop/NativeMethods.txt`, switch to `PInvoke.OpenProcessToken`/`PInvoke.GetTokenInformation`. Note CsWin32's generated signatures use safe handles and the `Windows.Win32.Security.TOKEN_INFORMATION_CLASS`/`TOKEN_ELEVATION_TYPE` types rather than the hand-rolled `IntPtr`s and the private `TOKEN_INFORMATION_CLASS` enum this file currently declares — expect signature adaptation, not a drop-in rename. Verify with a build, per CLAUDE.md's "the build is the arbiter." **Correction (found in the Phase 5A final review, 2026-09-05): these two overloads are not duplicates.** `IsProcessElevated(int processId)` ignores its parameter entirely and always queries `Process.GetCurrentProcess()` — it reports WinTabber's own elevation regardless of which pid is passed. `IsProcessElevated(Process process)` correctly queries the given process. `InteropProxy.BringWindowToFront` calls the `int` overload with the *target* window's process id, so it always branches on WinTabber's own elevation state, not the target's — a live bug, not a design duplicate. The CsWin32 migration (T5.1) correctly preserved this bug rather than "fixing" it during a mechanical migration, which was the right call for that task. Tracked as its own defect, `.cleanup/tasks.md` T5.6 — do not "deduplicate" these two overloads by unifying them; fix the parameter being ignored, which is a behavior change to `BringWindowToFront` deserving its own review and smoke test, not folded into a test-coverage task. |
