...
var sched = RxApp.MainThreadScheduler;
sched.Schedule(() =>
{
    Console.WriteLine($"Main thread: {Environment.CurrentManagedThreadId}");
});
var o = Observable.Create<string>((obs) =>
{
    Console.WriteLine($"Subscribing on {Environment.CurrentManagedThreadId}");
    obs.OnNext("value");
    obs.OnCompleted();
...
Console.WriteLine($"starting {Environment.CurrentManagedThreadId}");
//await Task.Delay(500).ConfigureAwait(true);
Console.WriteLine($"subscribing {Environment.CurrentManagedThreadId}");
o.Subscribe(x => Console.WriteLine($"{x}: {Environment.CurrentManagedThreadId}"));
Console.WriteLine($"subscribed {Environment.CurrentManagedThreadId}");
